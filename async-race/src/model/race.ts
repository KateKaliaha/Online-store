...
async function startEngine(id: string): Promise<{velocity: number, distance: number}> {
...
async function stopEngine(id: string): Promise<{velocity: number, distance: number}> {
...
async function driveCar(id: string): Promise<{success: boolean}> {
...
export async function stopRace(id: string): Promise<void> {
  const startButton = document.getElementById(`start-btn-${id}`) as HTMLButtonElement;
  const finishButton = document.getElementById(`finish-btn-${id}`) as HTMLButtonElement;
...
export function animation(duration: number, id: string): Record<string, number> {
  const car = document.getElementById(`image-car-${id}`) as HTMLDivElement;
...
export async function raceAllCars(): Promise<{win: Car; time: number;}> {
  const promises:Promise<ResultPromise>[] = [...cars.map(async (car:Car) => await startRace((car.id as number).toString()))];
  const win = await winner(promises, cars.map((car: Car) => car.id as number));
...
async function winner(prom: Promise<ResultPromise>[], ind: number[]): Promise<{win: Car; time: number;}> {
  ...
  const result: {cars: Car; time: string;} = { cars: cars.find((car:Car) => ((car.id) as number).toString() === id) as Car, time: (timeRace / 1000).toFixed(2) };
  arrayNamesAndColorsWinners[result.cars.id as number] = { name: result.cars.name, color: result.cars.color };

  return ({ win: result.cars, time: +result.time });
}

export async function stopRaceAllCars(): Promise<void> {
