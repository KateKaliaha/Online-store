...
export function renderNumberCarAndPage (countCar:string, numberPage:number, element:HTMLElement) {
    const numberCarInGarage = document.createElement('div');
    numberCarInGarage.className = 'number-car';
    numberCarInGarage.innerHTML = `Garage (${countCar})`;
    element.appendChild(numberCarInGarage);

    const numberPageInGarage = document.createElement('div');
    numberPageInGarage.className = 'number-page';
    numberPageInGarage.innerHTML = `Page #${numberPage}`;
    element.appendChild(numberPageInGarage);
}

export function renderCar(carsArr:Array<Car>, element:HTMLElement) {
    carsArr.forEach((car) => {
        const fragment = document.createElement('div');
        const id = car.id;
        fragment.className = 'car';
        fragment.id = id.toString();
        fragment.setAttribute('data-id', `${id}`);
        fragment.innerHTML = `<div class="btn-change-car">
        ...
        width="60px" height="60px" viewBox="0 0 79.536 79.536" style="enable-background:new 0 0 79.536 79.536;"
        xml:space="preserve">
        <path style="fill:${car.color};" d="M15.532,56.706c-3.977,0-7.213-3.242-7.213-7.197c0-3.998,3.236-7.224,7.213-7.224
